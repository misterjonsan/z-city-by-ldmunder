	...
	GunManLoot = function(ply)
		for k,v in player.Iterator() do
				timer.Simple(1,function()
					local Accessories = v:GetNetVar("Accessories",{"none"})
					if istable(Accessories) then
						Accessories[1] = "stetson"
					else
						Accessories = "stetson"
					end
					local appearance = v.CurAppearance or hg.Appearance.GetRandomAppearance()
					appearance.AClothes = appearance.AClothes or {}
					appearance.AClothes.main = "formal"
					ApplyForceAppearance(v, appearance)
					v:SetNetVar("Accessories", Accessories)
...
MODE.Types.soe = {
	...
	GunManLoot = function(ply)
		...
		ply.organism.recoilmul = 1.0
		if IsValid(gun) and gun:GetClass() == "weapon_kar98" then
...
function MODE:SendTraitorDeathState(traitor, is_alive)
    local name = ""
    if traitor.CurAppearance and traitor.CurAppearance.AName then
        name = tostring(traitor.CurAppearance.AName)
    elseif traitor.CurAppearance and traitor.CurAppearance.Name then
        name = tostring(traitor.CurAppearance.Name)
    else
        name = tostring(traitor:Name() or traitor:Nick() or "unknown")
    end
...
net.Receive("HMCD_RequestTraitorStatuses", function(len, ply)
    ...
    for _, other_ply in ipairs(player.GetAll()) do
        if other_ply.isTraitor then
            ...
            net.Start("HMCD_TraitorDeathState")
            local otherName = ""
            if other_ply.CurAppearance and other_ply.CurAppearance.AName then
                otherName = tostring(other_ply.CurAppearance.AName)
            elseif other_ply.CurAppearance and other_ply.CurAppearance.Name then
                otherName = tostring(other_ply.CurAppearance.Name)
            else
                otherName = tostring(other_ply:Name() or other_ply:Nick() or "unknown")
            end
            net.WriteString(otherName or "unknown")
            net.WriteBool(is_alive)
            net.Send(ply)
        end
    end
    ...
    for idx, current_ply in pairs(all_players) do
        if(current_ply:Team() != TEAM_SPECTATOR)then
            ...
            if(current_ply.isTraitor)then
                local convar_name = MODE.ConVarName_SubRole_Traitor
                if(MODE.Type == "soe")then
                    convar_name = MODE.ConVarName_SubRole_Traitor_SOE
                end
                local sub_role_id = current_ply:GetInfo(convar_name) or ((MODE.Type == "soe") and "traitor_default_soe" or "traitor_default")
                sub_role = sub_role_id
            end

				if(current_ply.isGunner and not current_ply.GunManLootGiven)then
					MODE.Types[MODE.Type].GunManLoot(current_ply)
					current_ply.GunManLootGiven = true
				end
            ...
            if(sub_role)then
                if(current_ply.isTraitor)then
                    ...
                    if(!role_info)then
                        sub_role = (MODE.Type == "soe") and "traitor_default_soe" or "traitor_default"
                        role_info = MODE.SubRoles[sub_role]
                    end

                    if(current_ply.MainTraitor)then
                        local spawn_func = role_info and role_info.SpawnFunction
                        current_ply.SubRole = sub_role
                        if spawn_func then spawn_func(current_ply) end
                        net.Start("HMCD(SetSubRole)")
                            net.WriteString(current_ply.SubRole or "")
                        net.Send(current_ply)
                    end
                end
            else
                if(current_ply.isTraitor)then
