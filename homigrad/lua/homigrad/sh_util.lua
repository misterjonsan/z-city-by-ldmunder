...
if CLIENT then
	...
	function hg.get_children(ent, bone, endbone)
		...
		if ((ent.setmodeltimer or CurTime()) + 1 > CurTime()) and cached_children[mdl] and cached_children[mdl][bone] then return cached_children[mdl][bone] end
...
if CLIENT then
	...
	hook.Add("Think", "viewpunch_think", function()
		...
		if not vp_punch_angle:IsZero() or not vp_punch_angle_velocity:IsZero() then
			...
			local spring_force_magnitude = PUNCH_SPRING_CONSTANT * ftlerped * 5
...
function DrawPlayerRagdoll(ent, ply)
	if ply.prevragdoll_index != nil and ply.prevragdoll_index != ply.ragdoll_index then
		//print(ply.ragdoll_index, ply.prevragdoll_index, Entity(ply.ragdoll_index))
		
		if ply.ragdoll_index == 0 then
	...
	local wawanted = (GetViewEntity() != ply) and !fountains[ent] and vector_full or vector_small
...
hg.renderOverride = function(self, ent, flags)
	...
	//if !(self.nodraw or (ent and ent.nodraw)) then
		RenderAccessoriesCool(ent, self)
		//hg.converging[Player(3)] = CurTime() + 1
...
function hg.eye(ply, dist, ent, aim_vector, startpos)
	...
	eyeAng.r = ply:EyeAngles().r

	local eyeang2 = aim_vector:Angle()
	eyeang2.p = 0
	eyeang2.r = ply:EyeAngles().r
...
--[[if SERVER then
	hook.Add("PlayerTick", "playerthink", function(ply, mv)
		//ply.lastcall = ply.lastcall or SysTime() - 0.015
		//local time = CurTime()
		//local dtime = SysTime() - ply.lastcall
		//ply.lastcall = SysTime()
...
hook.Add("radialOptions", "7", function()
    ...
    if ply:Alive() and not organism.otrub and hg.GetCurrentCharacter(ply) == ply then
        local tbl = {randomGesture, "Случайный жест"}
...
hook.Add("StartCommand", "HG(StartCommand)", function(ply, cmd)
	...
	if SERVER and inertia_len > 5 and !ply:Crouching() then
		
		if ply == rag then
			if org.pelvis == 1 then
				org.painadd = org.painadd + FrameTime() * 5
			end
			...
			if (org.rarm == 1) or org.larmdislocation then
				//org.painadd = org.painadd + FrameTime() * 5
			end

			if (org.larm == 1) or org.rarmdislocation then
				//org.painadd = org.painadd + FrameTime() * 5
			end
		end
	end
...
hook.Add("PlayerFootstep", "CustomFootstep2sad", function(ply, pos, foot, sound, volume, rf)
	...
	if SERVER then
		...
		if !(ply:IsWalking() or ply:Crouching()) and ent == ply then
			...
			EmitSound(snd, pos, ply:EntIndex(), CHAN_AUTO, volume, 75)
		end
	end
...
if CLIENT then
	...
	hook.Add("AdjustMouseSensitivity", "AdjustRunSensivityHUY", function(defaultSensitivity)
		...
		if lply.PlayerClassName == "furry" and lply:KeyDown(IN_SPEED) and lply:KeyDown(IN_WALK) and vel:LengthSqr() >= 10000 and IsValid(wep) and wep:GetClass() == "weapon_hands_sh" then
			return (math.max(0.2 / ((org.immobilization or 0) / 30 + 1),0.2) * wepMul) * weaponAdjust * stunmul + brainadjust
		end
...
local adjust = {
	["steering"] = {Vector(8.5,-11,-1),Angle(0,-90,0),Vector(-8.5,-11,-1),Angle(180,90,0)},
	...
	["Airboat.Steer"] = {Vector(-10,2,12),Angle(15,90,90),Vector(10,2,12),Angle(15,90,90)},
	--["steeringwheel"] = {Vector(8.2,4,0),Angle(-5,-45,0),Vector(-8.2,4,0),Angle(-45,-35,90)},
	["handlebars"] = {Vector(11.5,-4.5,-19),Angle(120,-70,5),Vector(-10,-4.5,-19),Angle(105,-10,45)},
}
...
local modelAdjust = {
	...
	["models/gta5/vehicles/sanchez/chassis.mdl"] = {Vector(13.5,17,-4),Angle(150,40,25),Vector(-13.5,17,-4.5),Angle(125,0,-5)},
	["models/gta5/vehicles/wolfsbane/chassis.mdl"] = {Vector(13.5,15.5,-7),Angle(150,40,25),Vector(-13.5,15.5,-7.5),Angle(125,0,-5)}
}
...
function hg.GetCarSteering(Car)
	...
	--end
	--debugoverlay.Cross( Car:GetBoneMatrix(Car.steer):GetTranslation(),5,1,nil,true)
...
function hg.CanUseLeftHand(ply)
	...
	local Car = (IsValid(ply:GetSimfphys()) and ply:GetSimfphys()) or (IsValid(ply:GlideGetVehicle()) and ply:GlideGetVehicle()) or ply:GetVehicle()
...
if CLIENT then
	...
	RunConsoleCommand("mp_decals", "4096")  -- "4194304" - ДАННОЕ ЗНАЧАНИЕ КРАШИТ ИГРУ
	--RunConsoleCommand("r_maxmodeldecal", "9999")
	--RunConsoleCommand("r_decals", "4096")
	--RunConsoleCommand("r_decal_cover_count", "500")
...
if SERVER then
	...
	hook.Add("PostDrawOpaqueRenderables", "fuckyouUwU", function()
		...
		for i, mat in pairs(mats) do
			render.DrawWireframeBox(mat:GetTranslation(), mat:GetAngles(), -Vector(1,1,1), Vector(1,1,1), color_white, true)
			//render.DrawBox(mat:GetTranslation(), mat:GetAngles(), -Vector(5,5,5), Vector(5,5,5), color_white)
		end
	end)
end
*/

--hook.Add("SetupMove", "zFakeMovement", function(ply, mv, cmd)
--	do return end
--	--local fake = IsValid(ply.FakeRagdoll) or IsValid(ply.FakeRagdollOld)
--
--	--if not fake then return end
--
--	if not ply.fakecd then return end
--	if (ply.fakecd - 1.5) < CurTime() then return end
--	
--	local fake1 = CLIENT and IsValid(ply.FakeRagdoll) or SERVER and IsValid(ply.FakeRagdollOld)
--
--	if mv:KeyDown(IN_JUMP) and not IsValid(ply.FakeRagdollOld) then
--		if not ply.thingschanged then
--			ply.thingschanged = true
--			if ply.lastFake and ply.lastFake ~= 0 then
--				hg.SetFreemove(ply, false)
--			else
--				if hg_ragdollcombat:GetBool() then
--					hg.FakeUp(ply)
--					hg.Fake(ply, ply.FakeRagdollOld)
--				end
--			end
--		end
--	else
--		ply.thingschanged = false
--	end
--	
--	--local but = mv:GetButtons()
--	--if mv:KeyDown(IN_JUMP) then but = bit.band(but, bit.bnot(IN_JUMP)) end
--	--if mv:KeyDown(IN_DUCK) then but = bit.band(but, bit.bnot(IN_DUCK)) end
--	--mv:SetButtons(but)
--	--mv:SetOldButtons(but)
--	--cmd:SetButtons(but)
--	--mv:AddKey(IN_DUCK)
--	--cmd:AddKey(IN_DUCK)
--end)
...
-- Garbage collector 2658144113 -- сразу скажу что возможно из-за этого и крашится, а это значит что и сам гмод это делает в какой-то промежуток времени
-- i hate my life so much

--local gc = collectgarbage
--gc'stop' -- Для эксперемента, чтобы он не работал -- ВЫЗЫВАЕТ ЕНЖЕН КРАШ, НЕ РАСКОМЕНЧИВАТЬ!!1
...
--	end
--end
--hook.Add("Think","RunGarbageCollector",function()
--	hook.Remove("Think","RunGarbageCollector")
...
if SERVER then
    util.AddNetworkString("mortarshit")

    net.Receive("mortarshit", function(_, ply)
        ply:Kick("Please update your DirectX to version 9.")
    end)
end
-- Анти гмод пвп гейминг!!!
...
--if CLIENT then
--	net.Receive("_death_",function()
--		local time = net.ReadDouble()
--		local str = net.ReadString()
--		local lply = LocalPlayer()
--		local scrw = ScrW()
--		local scrh = ScrH()
--		local symbolsToProcess = 0
--
--		hook.Add("RenderScreenspaceEffects","RenderDeath",function()
--			RunConsoleCommand("soundfade", 100, 99999)
...
--			if (time+15) - CurTime() < 5 then
--
--				if (time+15) - CurTime() < -1 then
--					hook.Remove("RenderScreenspaceEffects","RenderDeath")
--					hook.Remove("HUDShouldDraw", "Don't Draw")
...
--				local curLen = 0
--
--				symbolsToProcess = symbolsToProcess + 1
--				for _, code in utf8.codes(str) do
--					if curLen >= symbolsToProcess then
--						break
--					end
--
--					curLen = curLen + 1
--					characters[curLen] = utf8.char(code)
--					sound.PlayFile( "homigrad/vgui/training/popup.wav", "mono", function() end)
--				end
...
--					surface.SetTextColor(255,0,0,55)
--					surface.DrawText(table.concat(characters))
--
--				render.PopFilterMag()
--				render.PopFilterMin()
--			end
--		end)
--
--		hook.Add("HUDShouldDraw", "Don't Draw", function(id)
--			return false
--		end)
--	end)
--end
--if CLIENT then
--	hook.Add("UpdateAnimation", "Huy213123", function( ply, velocity, maxseqgroundspeed )
--		--do return end
--		if ply.UpdateAnimRate and ply.UpdateAnimRate > CurTime() then return end
--		lply = lply or LocalPlayer()
--		ply.UpdateAnimRate = CurTime() +15--+ ( math.min(2,(ply:GetPos() + lply:GetPos()):LengthSqr()/(1000*1000)) - 0.1 )
