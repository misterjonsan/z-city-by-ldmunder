...
hook.Add("Player Think","health_armor_gordonthings",function(ply,ent)
...
elseif SERVER then
    ...
    hook.Add("Org Think","gordon_healing",function(ply, org)
        ...
        if ply.PlayerClassName == "Gordon" and ply:GetNetVar("HEVSuit") then
            ...
            if org.pneumothorax > 0 then
                org.needle = true
                ply:Notify("HEV suit has detected pneumothorax. Needle injected.",true,"needle_hev",0.5,function(ply)
                    net.Start("HEV_DAMAGE")
                        net.WriteString("hl1/fvox/automedic_on.wav")
                    net.Send(ply)
                end,hev_color)
            ...
            if org.pain > 30 then
                if (org.NextMorphineInject or 0) < CurTime() then
                    ...
                    local need = math.min(ply.HEV.Morphine, org.pain / 80)
                    local old = org.analgesia
                    org.analgesia = math.max(math.min(org.analgesia + need, org.pain / 80, 1), org.analgesia)
            ...
            if (org.pulse < 40) or (org.blood < 3000) or (org.o2[1] < 10) then
                ...
                if (org.BloodThink or 0) < CurTime() then
                    ...
                    if org.blood < 4000 then
                        ...
                        ply.HEV.Medicine = math.max( ply.HEV.Medicine - needed / 6, 0 )
    ...
    hook.Add("HomigradDamage", "HEV_Medical",function(ply, dmgInfo, hitgroup, entharm, hitBoxs, inputHole)
        if ply.PlayerClassName == "Gordon" and ply:GetNetVar("HEVSuit") then
            timer.Simple(0,function()
                ...
                timer.Create("HEV_CheckHP",10,1,function()
                    ...
                    local needed = 100 - ply:Health()
                    ply:SetHealth(math.min(ply:Health() + ply.HEV.Medicine, 100))
                    ply.HEV.Medicine = math.max( ply.HEV.Medicine - needed, 0 )
                ...
                if ply.organism.bleed > 0 and (#ply.organism.wounds > 0) then
                    if (not ply.HEV.BloodLossDetect) or ply.HEV.BloodLossDetect < CurTime() then
                        net.Start("HEV_DAMAGE")
                            net.WriteString("hl1/fvox/blood_loss.wav")
                        net.Send(ply)

                        ply.HEV.BloodLossDetect = CurTime() + 10
                    end
                    ...
                    timer.Create("HEV_StopBleed",5,1,function()
                        ...
                        ply.organism.bleed = math.max(ply.organism.bleed - ply.HEV.Medicine,0)
                        ...
                        net.Start("HEV_DAMAGE")
                            net.WriteString("hl1/fvox/bleeding_stopped.wav")
                        net.Send(ply)
                    end)
                end
            end)
