...
local function legs(org, bone, dmg, dmgInfo, key, boneindex, dir, hit, ricochet)
	...
	if dmg >= 1 and (!dmgInfo:IsDamageType(DMG_CLUB+DMG_CRUSH+DMG_FALL) or math.random(3) != 1) then
		...
		if org.isPly then org.owner:Notify(broke_leg[math.random(#broke_leg)], 1, "broke"..key, 1, nil, nil) end
		...
		if org.isPly then org.owner:Notify(dislocated_leg[math.random(#dislocated_leg)], 1, "dislocated"..key, 1, nil, nil) end
...
local function arms(org, bone, dmg, dmgInfo, key, boneindex, dir, hit, ricochet)
	...
	if dmg >= 1 and (!dmgInfo:IsDamageType(DMG_CLUB+DMG_CRUSH+DMG_FALL) or math.random(3) != 1) then
		...
		if org.isPly then org.owner:Notify(broke_arm[math.random(#broke_arm)], 1, "broke"..key, 1, nil, nil) end
		...
		if org.isPly then org.owner:Notify(dislocated_arm[math.random(#dislocated_arm)], 1, "dislocated"..key, 1, nil, nil) end
...
hook.Remove("CanListenOthers", "CantSayWithBrokenJaw")
