...
input_list.liver = function(org, bone, dmg, dmgInfo)
	local oldDmg = org.liver[1]
	local prot = math.max(0.3 - org.liver[1],0)
	
	hg.AddHarmToAttacker(dmgInfo, (org.liver[1] - oldDmg) * 3, "Liver damage harm")
	
	org.shock = org.shock + dmg * 20
	org.painadd = org.painadd + dmg * 35
	
	org.liver[1] = math.min(org.liver[1] + dmg, 1)
	local harmed = (org.liver[1] - oldDmg)
...
input_list.brain = function(org, bone, dmg, dmgInfo)
	...
	//if org.brain >= 0.7 then org.posturing = math.random(10) == 1 end
...
local function hitArtery(artery, org, dmg, dmgInfo, boneindex, dir, hit)
	...
	if dmgInfo:IsDamageType(DMG_SLASH) and math.random(5) != 1 then return end
	...
	table.insert(org.arterialwounds,{arterySize[artery], localPos, localAng, bonea, CurTime(), dir2 * 100, artery})
