...
bloodparticles_hook[3] = function(anim_pos)
    ...
    for i = 1,#bloodparticles2 do
        local part = bloodparticles2[i]
...
bloodparticles_hook[4] = function(mul)
    ...
    for i = #bloodparticles2, 1, -1 do
        local part = bloodparticles2[i]
        if not part then table_remove(bloodparticles2, i) continue end
		...
		if radiusSqr < hitPos:LengthSqr() then table_remove(bloodparticles2, i) continue end

        if result.Hit or part[7] - time <= 0 then
            table_remove(bloodparticles2, i)
