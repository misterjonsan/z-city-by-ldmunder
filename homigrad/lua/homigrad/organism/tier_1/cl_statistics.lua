net.Receive("organism_send", function()
	...
	if add and org.owner.organism and org.owner.new_organism then
		table.Merge(org.owner.organism, org)
		table.Merge(org.owner.new_organism, org)
...
hook.Add("Player Death","removeorg",function(ply)
	ply.organism = nil
	...
	for i, v in pairs(list) do
		...
		if type(v) == "table" then
			if type(v[1]) == "table" then
				...
				-- Обработка табличных значений (liver, stamina и т.д.)
				if type(value) == "table" then 
					-- Для таблиц типа {damage, threshold} берём первый элемент
					if isnumber(value[1]) then
						value = value[1]
					-- Для таблиц типа stamina с полями показываем как есть
...
hook.Add("HUDPaint", "homigrad-organism-debug", function()
	...
	-- Интерполируем данные для актуальности
	if new_organism and not table.IsEmpty(new_organism) then
		LerpVariables(FrameTime() * 5, organism, new_organism)
	end
	...
	-- Проверяем существование функции и игрока ДО вызова
	if not hg.eyeTrace or not IsValid(LocalPlayer()) then return end
	
	local tr = hg.eyeTrace(LocalPlayer(), 10000)
	if not tr or not IsValid(tr.Entity) then return end

	local trent = tr.Entity
	if not trent.organism or table.IsEmpty(trent.organism) then return end
...
hook.Add("Player Death","wowww",function(ply)
