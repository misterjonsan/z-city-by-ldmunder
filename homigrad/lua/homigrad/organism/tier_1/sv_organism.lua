...
local function send_organism(org, ply)
	...
	-- Дополнительные поля для developer mode
	sendtable.spine1 = org.spine1
	sendtable.spine2 = org.spine2
	sendtable.spine3 = org.spine3
	sendtable.chest = org.chest
	sendtable.skull = org.skull
	sendtable.jaw = org.jaw
	sendtable.heart = org.heart
	sendtable.heartstop = org.heartstop
	sendtable.stomach = org.stomach
	sendtable.liver = org.liver
	sendtable.intestines = org.intestines
	sendtable.lungsL = org.lungsL
	sendtable.lungsR = org.lungsR
	sendtable.trachea = org.trachea
	sendtable.pneumothorax = org.pneumothorax
	sendtable.needle = org.needle
	sendtable.internalBleed = org.internalBleed
	sendtable.internalBleedHeal = org.internalBleedHeal
	sendtable.rarmartery = org.rarmartery
	sendtable.larmartery = org.larmartery
	sendtable.rlegartery = org.rlegartery
	sendtable.llegartery = org.llegartery
	sendtable.spineartery = org.spineartery
	sendtable.painkiller = org.painkiller
	sendtable.painadd = org.painadd
	sendtable.avgpain = org.avgpain
	sendtable.naloxone = org.naloxone
	sendtable.tranquilizer = org.tranquilizer
	sendtable.wantToVomit = org.wantToVomit
	sendtable.satiety = org.satiety
	sendtable.thiamine = org.thiamine
	sendtable.CO = org.CO
	sendtable.COregen = org.COregen
	sendtable.hemotransfusionshock = org.hemotransfusionshock
	sendtable.assimilated = org.assimilated
	sendtable.fearadd = org.fearadd
	sendtable.likely_phrase = org.likely_phrase
	sendtable.alive = org.alive
	sendtable.health = org.health
...
hook.Add("Org Think", "Main", function(owner, org, timeValue)
	...
	-- Отключаем организм если у игрока активна защита от спавн-киллинга
	if owner.organismDisabled then return end
	...
	if isPly then
		...
		if not org.alive and owner:Alive() then 
			-- Set last attacker for PlayerDeath hook
			if IsValid(owner.lastAttacker) and (owner.lastAttackerTime or 0) > CurTime() - 10 then
				owner:SetNWEntity("LastAttacker", owner.lastAttacker)
			end
			owner:Kill() 
		end
	end
...
local function fixlimb(org, key)
	if math.random(100) > (60 - (org.analgesia * 50 + org.painkiller * 15)) then
...
concommand.Add("hg_toggle_rage", function(ply, cmd, args)
	...
	if not IsValid(target) or not target:Alive() then
		ply:ChatPrint("Target player is not valid or not alive!")
		return
	end
	...
	if org.rage > 0 or org.anger > 0 then
		org.rage = 0
		org.anger = 0
		ply:ChatPrint("Disabled rage and anger for " .. target:Nick())
		if target ~= ply then
			target:ChatPrint("Your rage and anger have been disabled by " .. ply:Nick())
		end
	else
		org.rage = 1
		org.anger = 1
		ply:ChatPrint("Enabled rage and anger for " .. target:Nick())
		if target ~= ply then
			target:ChatPrint("Your rage and anger have been enabled by " .. ply:Nick())
		end
	end
	
	hg.SendOrganismData(target, true)
end)