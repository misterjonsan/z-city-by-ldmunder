...
hook.Add("radialOptions", "DislocatedArm", function()
    if !lply:Alive() or !lply.organism or lply.organism.otrub then return end
	if (lply.tried_fixing_limb or 0) > CurTime() then return end
	local org = lply.organism
	if org.pain > 60 then return end
			...
			if wound[5] + beatsPerSecond < time then
				...
				if seen then
					...
					if mat and wound[2] and wound[3] then
						local matPos, matAng = mat:GetTranslation(), mat:GetAngles()
						if matPos and matAng then
							pos = LocalToWorld(wound[2], wound[3], matPos, matAng)
						end
					end
					
					if not pos and bonePos and wound[2] and wound[3] then
						pos = LocalToWorld(wound[2], wound[3], bonePos, boneAng or angle_zero)
					end
					
					if not pos and bonePos then
						pos = bonePos
					end
	...
	if arterialwounds and #arterialwounds > 0 then
		for i, wound in pairs(arterialwounds) do
			...
			if wound[5] + addtime < time then
				local bonePos, boneAng = ent:GetBonePosition(wound[4])
				wound[5] = time
				if seen then
					...
					local pos = nil
					local ang = boneAng or angle_zero
					...
					if pos then
						local dir = wound[6]
						local len = dir and dir:Length() or 100
						len = len * (org.pulse or 70) / 70
						
						local bloodDir = vector_origin
						if dir and dir:Length() > 0 then
							local _, dirAng = LocalToWorld(vector_origin, dir:Angle(), vector_origin, ang)
							bloodDir = -dirAng:Forward() * len
						else
							bloodDir = ang:Right() * len * 0.5 + ang:Forward() * len * 0.3
						end
						
						if bit.band(util.PointContents(pos), CONTENTS_WATER) == CONTENTS_WATER then
							hg.addBloodPart2(pos, VectorRand(-5, 5), nil, nil, nil, nil, true, nil, ent)
						else
							hg.addBloodPart(pos, VectorRand(-1, 1) * (org.pulse or 70) / 70 + bloodDir * 5 * (math.abs(math.sin(CurTime() * 2) + math.cos(CurTime() * (5 + i * 2)) + math.sin(CurTime() * (1 + i))) * 0.6 + math.sin(CurTime() * 2) + 4) * 0.1 + bloodDir:Angle():Right() * 25 * math.sin(CurTime() * 2) * math.cos(CurTime() * 4) + ang:Up() * 25 * math.sin(CurTime() * 3) * math.cos(CurTime() * 1) + VectorRand(-1, 1) * (org.pulse or 70) / 70, nil, size, size, true, nil, ent)
						end
					end
				end
			end
