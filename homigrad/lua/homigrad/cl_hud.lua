...
hook.Add( "PlayerBindPress", "PlayerBindPressExample2huy", function( ply, bind, pressed )
		...
		if input.IsKeyDown(KEY_C) and IsValid(lply.FakeRagdoll) and lply:IsSuperAdmin(true) then return end 

        if input.IsKeyDown(KEY_E) and GetConVar("hg_qmenu_open"):GetInt() == 1 and !IsValid(lply.FakeRagdoll) then 
            return nil 
		else
			if pressed then
                CreateRadialMenu()
