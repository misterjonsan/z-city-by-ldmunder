...
local colors = {}
colors.secondary = Color(139,0,0,162)
colors.mainText = Color(255,255,255,255)
colors.secondaryText = Color(110,110,110,125)
colors.selectionBG = Color(2,128,29,161)
colors.highlightText = Color(255,0,0)
...
local function PlaySafeSound(soundPath)
    if file.Exists("sound/" .. soundPath, "GAME") then
        ...
        -- Альт звук если ориг отсутствует
        surface.PlaySound("buttons/button15.wav")
        print("[WARNING] Sound file missing: " .. soundPath)
    end
end
...
function PANEL:Paint(w,h)
    surface.SetDrawColor(0,0,0,241)
    surface.DrawRect(0, 0, w, h)

    surface.SetDrawColor(204, 10, 10, 150)
...
function PANEL:PostInit()
    ...
    local function OnAccessorUnhover()
        ...
        hoveredAccessor = nil
        -- Возвращаем к дефолтной позиции камеры
    ...
    hatSelector:SetText("Hat attachment")
    ...
    function hatSelector:DoClick()
        ...
        for k, v in pairs(hg.Accessories) do
            ...
            function ico:OnCursorEntered()
                ...
                -- Показываем предпросмотр аксессуара на персонаже
            ...
            function ico:OnCursorExited()
                OnAccessorUnhover()
                -- Убираем предпросмотр аксессуара
            ...
            function ico:DoClick()
                ...
                PlaySafeSound("buttons/button15.wav")
                -- При выборе возвращаем камеру к дефолтной позиции
                viewer.TargetLookAt = defaultLookAt
                viewer.TargetCamPos = defaultCamPos
                viewer.Entity.AccessorySkins = main.AppearanceTable.AAccessorySkins
                viewer.Entity.AccessoryBodygroups = main.AppearanceTable.AAccessoryBodygroups
    ...
    faceSelector:SetText("Face attachment")
    ...
    -- Attachment skins/bodygroups picker
    ...
    function attSkinSelector:DoClick()
        ...
        -- Build a list of currently chosen attachments
        for _, accessor in ipairs(main.AppearanceTable.AAttachments or {}) do
            if accessor and accessor ~= "none" and hg.Accessories[accessor] then
                ...
                -- Determine skin count by probing model
