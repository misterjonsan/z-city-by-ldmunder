...
hook.Add("HG.InputMouseApply", "asdasd2", function(tbl)
	...
	if ishgweapon(wep) and wep.weight > 1 then
		ViewPunch2(Angle(-y / 50 / 8, x / 50 / 8, 0) * math.min(wep.weight, 1.5) / 4)
	end
...
CalcView = function(ply, origin, angles, fov, znear, zfar)
	...
	if ply:InVehicle() then
		...
		//if ply:GetVehicle().GetThirdPersonMode and ply:GetVehicle():GetThirdPersonMode() then
	...
	if ply:Alive() then
		...
		//	if not ply.OldRagdoll:GetManipulateBoneScale(ply.OldRagdoll:LookupBone("ValveBiped.Bip01_Head1")):IsEqualTol(vecZero,0.11) then
		//		ply.OldRagdoll:ManipulateBoneScale(ply.OldRagdoll:LookupBone("ValveBiped.Bip01_Head1"), firstPerson and vecPochtiZero or vecFull)
		//	end
		//end
	end
