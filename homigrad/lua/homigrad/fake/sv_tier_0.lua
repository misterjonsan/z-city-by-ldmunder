...
function hg.FakeUp(ply, forced, respawn_like)
		...
		if ragdoll.welds then
			...
			for i, weld in pairs(ragdoll.welds) do
				if IsValid(weld) then
					local bon = weld:GetTable().Bone1
					timer.Simple(0.1, function()
						constraint.Weld(ragdoll, IsValid(veh:GetParent()) and veh:GetParent() or veh, bon, 0, 800, false, false)
					end)
					weld:Remove()
				end
			end
	...
	local posit
	if ply.exit_vehicle_pos then
		posit = ply.exit_vehicle_pos
		ply.exit_vehicle_pos = nil
	else
		posit = ent:GetBoneMatrix(ent:LookupBone("ValveBiped.Bip01_Pelvis")):GetTranslation()
	end
...
hook.Add("CanPlayerEnterVehicle","fake_enterveh",function(ply) return not IsValid(ply.FakeRagdoll) end)
...
function hg.GetUpPos(target,pos,tries,starttries)
	...
	if tries <= 0 then
		...
		local vec = #poses > 0 and Vector(poses[1][1],poses[1][2],poses[1][3])
		poses = {}
		if vec then
			local tr = util.TraceHull({
				start = vec,
				endpos = vec - Vector(0,0,128),
				mins = mins,
				maxs = maxs,
				filter = target
			})
