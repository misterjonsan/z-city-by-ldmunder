...
hook.Add("Think", "Fake", function()
	...
	for ply, ragdoll in pairs(hg.ragdollFake) do
		...
		if ply.organism and ply.organism.wounds and not table.IsEmpty(ply.organism.wounds) and org.canmove and (ply.fakecd and (ply.fakecd + 1) > CurTime()) then
			...
			if tracehuy.Hit then
				...
				if not ply:KeyDown(IN_USE) then
					shadowControl(ragdoll, 10, 0.001, nil, nil, nil, ragdoll:GetPhysicsObjectNum(realPhysNum(ragdoll,8)):GetPos(), 10, 10)
					shadowControl(ragdoll, 1, 0.001, nil, nil, nil, ragdoll:GetPhysicsObjectNum(realPhysNum(ragdoll,8)):GetPos(), 10, 10)
					...
					shadowControl(ragdoll, 11, 0.001, nil, nil, nil, spine:GetPos() + spine:GetAngles():Forward() * 50, 10, 10)
					shadowControl(ragdoll, 8, 0.001, nil, nil, nil, spine:GetPos() + spine:GetAngles():Forward() * 50, 10, 10)
				end
			end
		end
		...
		if not wep.RagdollFunc then
			...
			if ply:KeyDown(IN_ATTACK2) or (ishgweapon(wep) and ply:KeyDown(IN_USE)) then// || ply:InVehicle() then
				if org.canmove then
	...
	--
		...
		if not inmove and org.canmove and ply.FakeRagdoll == ragdoll then
			if ply:KeyDown(IN_DUCK) then
				--[[if ply:InVehicle() and  then
					 ply:ExitVehicle() 
					 local rag = ply.FakeRagdoll
					 rag:SetPos(ply.exit_vehicle_pos)
				end]] -- кто хочет доделайте, я чет устал вообще от гарриса, тут нехуй делать: проверку на мотик и позицию выхода которая может на пару юнитов выше быть от игрока чтобы слетать
				...
				if lthigh and rthigh then
					...
					legAng2:RotateAroundAxis(angles:Right(), 70)
				 ...
				 -- а еще пидорасы, упоминайте кто эту хуйню сделал. я вообще в ахуе с людей что говорят "гптшный дропкик" нейронка блять была использована рубичем, и то когда был онли дропкик, я молчу про зград дропкик где взят мой но в конце система подката где каждая 2 строчка это коммент с разъеснением  
				 -- удачи твины✌️ 
				end
			end
		end
