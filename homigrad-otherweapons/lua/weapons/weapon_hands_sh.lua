	...
	if SERVER then
		...
		elseif IsValid(tr.Entity) and tr.Entity:IsPlayer() then
			...
			if Dist < self.ReachDistance then
				...
				local ownerOrg = self:GetOwner().organism
				-- Can grab players if: superfighter, sc_infiltrator, furry (vs non-furry), or has anger > 0
				if ownerOrg.superfighter or self:GetOwner().PlayerClassName == "sc_infiltrator" or (self:GetOwner().PlayerClassName == "furry" and tr.Entity.PlayerClassName ~= "furry") or (ownerOrg.anger and ownerOrg.anger > 0) then
		...
		if SERVER then
			...
			if ply:KeyDown(IN_ATTACK) and !ply.organism.superfighter and !(org and ply.PlayerClassName == "furry" and org.owner.PlayerClassName != "furry") then
				...
				if bone != "ValveBiped.Bip01_Spine2" or not trace.Hit then
					-- Rage mode throw multiplier
					local rageMul = 1
					if ply.organism.rage and ply.organism.rage > 0 then
						rageMul = 1 + ply.organism.rage * 12 -- 0 rage = 1x, 1 rage = 5x
					end
					phys:ApplyForceCenter(ply:GetAimVector() * math.min(5000, phys:GetMass() * 800) * rageMul)
					self:SetCarrying()
				end
			...
			if ply:KeyDown(IN_ATTACK) and ply.organism.superfighter then
				...
				local rageMul = 1
				if ply.organism.rage and ply.organism.rage > 0 then
					rageMul = 1 + ply.organism.rage * 6 -- 0 rage = 1x, 1 rage = 5x
				end
				phys:ApplyForceCenter(ply:GetAimVector() * 60000 * self.Penetration * rageMul)
				self:SetCarrying()
			end
		end
...
function SWEP:AttackFront(special_attack, rand)
	...
	if IsValid(Ent) or (Ent and Ent.IsWorld and Ent:IsWorld()) then
		if string.find(Ent:GetClass(),"break") and Ent:GetBrushSurfaces()[1] and string.find(Ent:GetBrushSurfaces()[1]:GetMaterial():GetName(),"glass") then
			...
			//if math.random(1,8) == 8 and Ent:Health() < 250 then
	...
	-- Rage mode damage multiplier (1x to 5x based on rage level)
	local rageMul = 1
	if owner.organism.rage and owner.organism.rage > 0 then
		rageMul = 1 + owner.organism.rage * 2 -- 0 rage = 1x, 1 rage = 5x
		Mul = Mul * rageMul
	end
	...
	if IsValid(Phys) then
		-- Rage mode throw multiplier (stronger than damage multiplier)
		local throwMul = 1
		if owner.organism.rage and owner.organism.rage > 0 then
			throwMul = 1 + owner.organism.rage * 4 -- 0 rage = 1x, 1 rage = 5x
		end
		if Ent:IsPlayer() then Ent:SetVelocity(AimVec * SelfForce * 1.5 * (owner.organism.superfighter and 5 or 1) * throwMul) end
		Phys:ApplyForceOffset(AimVec * 5000 * Mul * throwMul, HitPos)
