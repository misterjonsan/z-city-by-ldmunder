...
if CLIENT then
    function MissingConvMsg()
        -- Function is empty to suppress the popup message
    end
end 
...
elseif SERVER && ( !file.Exists("convenience/adam.lua", "LUA") && !conv ) then
    hook.Add("PlayerInitialSpawn", "convenienceerrormsg", function( ply )
        ply:SendLua('MissingConvMsg()')
    end)


    hook.Add("PlayerInitialSpawn", "convenienceerrormsg_zbase", function( ply )
        ply:SendLua(
            'chat.AddText(Color(255, 0, 0), "WARNING: ZBase WILL not work as intended without the CONV library!")' ..
...
if SERVER then
    ZBaseDidConsoleLogo = true
    -- Console logo removed
end
